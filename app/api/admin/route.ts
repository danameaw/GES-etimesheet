...
export async function GET(req: NextRequest) {
  ...
  if (view === "project") {
    ...
    for (const ts of timesheets) {
      for (const entry of ts.entries) {
        ...
        // Avoid duplicate employee per project
        ...
        if (exists) {
          ...
          proj.employees.push({
            ...
            timesheetId: ts.id,
            status:      ts.status,
            totalHrs:    ts.entries.reduce((s, e) => s + e.totalHrs, 0),
  ...
  return NextResponse.json({
    summary: { total: allEmployees.length, submitted, draft, missing, weekStart, weekEnd },
