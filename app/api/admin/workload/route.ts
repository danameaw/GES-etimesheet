...
// Returns department workload for given month vs standard hours
export async function GET(req: NextRequest) {
  ...
  // For ges_management: only their own department
  ...
  // Fetch employee-level plans for this month
  ...
  // Standard hours: count Mon–Fri days in month minus company holidays
  ...
  const deptMap = new Map<string, Map<string, { employee: any; totalPlanned: number; projects: any[] }>>();
  for (const p of plans) {
    const dept  = p.employee.department;
    const empId = p.employee.id;
    if (!deptMap.has(dept)) deptMap.set(dept, new Map());
    const empMap = deptMap.get(dept)!;
    if (!empMap.has(empId)) empMap.set(empId, { employee: p.employee, totalPlanned: 0, projects: [] });
