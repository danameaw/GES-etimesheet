/**
 * Production seed script (plain JS — runs without ts-node)
 * Runs only if database is empty (safe to call on every start)
 */
...
async function main() {
  ...
  if (count > 0) {
    console.log(`[seed] DB already has ${count} employees — skipping.`);
    return;
  }
  ...
  for (const e of employees) {
    await prisma.employee.upsert({ where: { employeeId: e.employeeId }, update: {}, create: e });
  }
  ...
  for (const p of projects) {
    await prisma.project.upsert({ where: { projectNumber: p.projectNumber }, update: { managerId: p.managerId }, create: p });
  }
  ...
  for (const t of taskCodes) {
    await prisma.taskCode.upsert({ where: { code: t.code }, update: {}, create: t });
  }

  console.log("[seed] Done: 20 employees, 26 projects, 48 task codes.");
}
